const usersModel = require("../Models/usersModel"),
  // courseModel = require("../models/index"),
  usersController = {},
...
usersController.create = async (req, res) => {
  ...
  try {
    ...
    const data = await DB.create(usersModel, paylod);
...
// usersController.readCourses = async (req, res) => {
//   try {
//     console.log("object|", await courseModel.sequelize["courses"].findAll());
//     // const data = await courseModel.;
//     // const data = await DB.findAll(courseModel);
//     Util.dataValidation(res, data, controllerName);
//   } catch (error) {
//     log(controllerName, Util.readMessage(controllerName, error));
//     sendError(res, error, Util.readMessage(controllerName, error));
...
async function updateUser(email, firebase_token) {
  usersModel
    .update({ firebase_token: firebase_token }, { where: { email } })
    .then((rowAffected) => {
      if (rowAffected > 0)
        log(
          controllerName,
